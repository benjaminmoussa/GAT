...
def vectorizeCSV(bag, filepath):
    examples = pd.read_csv(filepath, encoding = "ISO-8859-1")
...
def createVector(bag, word_list):
    ...
    if len(vector) != 1167:
        print('meow')
    return vector
def createVectorCameo(data):
    cameo_dict = dict()
    for index, row in data.iterrows():
        if row['c_number'] not in cameo_dict:
            cameo_dict[row['c_number']] = row['c_event']
        else:
            continue
    return cameo_dict
...
##create bag of words############################
#path = '/Users/Moussa/documents/LUCAS/cameo.csv'
#filepath = '/Users/Moussa/documents/LUCAS/All_Samples.csv'
#bag = makeBag(path)
#bag = Series(bag)
#bag.to_csv('/Users/Moussa/documents/LUCAS/BagOfWords.csv', index = False)
##################################################
...
#vector.to_pickle('/Users/Moussa/documents/LUCAS/AllSampleVectors.csv')
##for index, row in vector.iterrows():
#    ex = row['ex']
#    cv = row['c_event']
#    print(ex.count(1),cv.count(1))